...
class WorkOrderApplyView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            ...
            type = forms.cleaned_data["type"]   # type 0
            title = forms.cleaned_data["title"]  # title:u'发达城市'
            order_contents = forms.cleaned_data["order_contents"]  # order_contents:u"大法师"
            assign_to = forms.cleaned_data["assign_to"]  # assign_to: u'3'

            work_order = WorkOrder()   # work_order: 发达省份
            ...
            return render(request, 'order/workorder_apply.html', {'forms': forms, 'errmsg': '工单填写格式错误'})
...
class WorkOrderHistoryView(LoginRequiredMixin, PaginationMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # 显示所有处理完毕的工作
        queryset = queryset.filter(status__gte=2)     # gte
